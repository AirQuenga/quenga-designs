...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-11 bg-slate-900 px-4 text-left align-middle text-xs font-semibold uppercase tracking-wide text-slate-50 first:rounded-tl-md last:rounded-tr-md [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
